...
A basic mean reversion trading strategy that:
- Buys when price is below the moving average (oversold)
- Sells when price is above the moving average (overbought)
- Uses Bollinger Bands for signal generation
"""
...
class MeanReversionStrategy:
    ...
    def calculate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            ma = data['close'].rolling(window=self.parameters['window']).mean()
            std = data['close'].rolling(window=self.parameters['window']).std()
            
            upper_band = ma + (self.parameters['std_dev'] * std)
            lower_band = ma - (self.parameters['std_dev'] * std)
...
def create_strategy(parameters: Dict[str, Any] = None) -> MeanReversionStrategy:
...
if __name__ == "__main__":
    # Test the strategy
    print("Mean Reversion Strategy loaded successfully!")
